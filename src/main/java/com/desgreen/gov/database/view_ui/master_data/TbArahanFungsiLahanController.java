...
@Controller
@ComponentScan
public class TbArahanFungsiLahanController {
    ...
    @RequestMapping("/master/arahanfungsilahan")
    public String listIndex(final Model viewModel) {

        // final Authentication auth = SecurityContextHolder.getContext().getAuthentication();

  
        final TbArahanFungsiLahan newDomain =new TbArahanFungsiLahan();
        ...
        viewModel.addAttribute("allData", arahanFungsiLahanRepository.findAll());
        // viewModel.addAttribute("allData", new ArrayList<TbArahanFungsiLahan>());

        logger.info("# Form Task");
      
        return "master/arahanfungsilahan/arahanfungsilahan_list"; 
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = {"/master/arahanfungsilahan_form/save_process"}, method = RequestMethod.POST)
    public String saveProcess(@ModelAttribute("domain") final TbArahanFungsiLahan domain, @ModelAttribute("domainDetil1") final TbArahanFungsiLahan domainDetil1, final RedirectAttributes redirectAttributes) {
        if (domain.getTempInt1()==0) {
            TbArahanFungsiLahan newDomain = new TbArahanFungsiLahan();
            ...
            if (arahanFungsiLahanRepository.save(newDomain) != null) {
            ...
            return "redirect:/master/arahanfungsilahan/edit_form/" + newDomain.getId();

        }else if (domain.getTempInt1()==1) {
            TbArahanFungsiLahan domainUpdate = arahanFungsiLahanRepository.findById(domain.getId() );
            ...
            arahanFungsiLahanRepository.save(domainUpdate);

            redirectAttributes.addFlashAttribute("saveUser", "success");
            return "redirect:/master/arahanfungsilahan/edit_form/" + domain.getId();
        }
       
        
        return "redirect:/master/arahanfungsilahan/";
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = "/master/arahanfungsilahan/{operation}/{id}", method = RequestMethod.GET)
    public String toFormOperation(@PathVariable("operation") final String operation,
        ...
        logger.info("/master/arahanfungsilahan/operation: {} ", operation);

       if (operation.equals("delete")) {
            try {
                TbArahanFungsiLahan domainToDelete = arahanFungsiLahanRepository.findById(id);        
                // hati-hati kecamatanRepository.deleteAll(domainToDelete.getListDesa() );
                arahanFungsiLahanRepository.delete(domainToDelete);               
            ...
            if (newDomain != null) {
                ...
                return "master/arahanfungsilahan/arahanfungsilahan_form";
            }

        } else if (operation.equals("edit_form")) {

            TbArahanFungsiLahan domain = arahanFungsiLahanRepository.findById(id);
            if (domain != null) {
                domain.setTempInt1(1); //0.New Form, 1.Edit Form, 3.Delete
                //Password tidak bisa di decode ya?


                // TbArahanFungsiLahanRoles domainDetil1 = new TbArahanFungsiLahanRoles();
                // TbArahanFungsiLahanRoles domainDetil2 = new TbArahanFungsiLahanRoles();
                // // for (TbArahanFungsiLahanRoles domainDetil: domain.getFuserRoles()) {
    ...
    @RequestMapping("/master/arahanfungsilahan/perbaikan")
    @ResponseBody
