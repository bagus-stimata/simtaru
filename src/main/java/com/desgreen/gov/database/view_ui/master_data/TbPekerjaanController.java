...
@Controller
@ComponentScan
public class TbPekerjaanController {
    ...
    @RequestMapping("/master/pekerjaan")
    public String listIndex(final Model viewModel) {

        // final Authentication auth = SecurityContextHolder.getContext().getAuthentication();

  
        final TbPekerjaan newDomain =new TbPekerjaan();
        ...
        viewModel.addAttribute("allData", pekerjaanRepository.findAll());
        // viewModel.addAttribute("allData", new ArrayList<TbPekerjaan>());

        logger.info("# Form Task");
      
        return "master/pekerjaan/pekerjaan_list"; 
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = {"/master/pekerjaan_form/save_process"}, method = RequestMethod.POST)
    public String saveProcess(@ModelAttribute("domain") final TbPekerjaan domain, @ModelAttribute("domainDetil1") final TbPekerjaan domainDetil1, final RedirectAttributes redirectAttributes) {
        if (domain.getTempInt1()==0) {
            TbPekerjaan newDomain = new TbPekerjaan();
            ...
            if (pekerjaanRepository.save(newDomain) != null) {
            ...
            return "redirect:/master/pekerjaan/edit_form/" + newDomain.getId();

        }else if (domain.getTempInt1()==1) {
            TbPekerjaan domainUpdate = pekerjaanRepository.findById(domain.getId() );
            ...
            pekerjaanRepository.save(domainUpdate);

            redirectAttributes.addFlashAttribute("saveUser", "success");
            return "redirect:/master/pekerjaan/edit_form/" + domain.getId();
        }
    ...
    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = "/master/pekerjaan/{operation}/{id}", method = RequestMethod.GET)
    public String toFormOperation(@PathVariable("operation") final String operation,
        ...
        logger.info("/master/pekerjaan/operation: {} ", operation);

       if (operation.equals("delete")) {
            try {
                TbPekerjaan domainToDelete = pekerjaanRepository.findById(id);        
                // hati-hati kecamatanRepository.deleteAll(domainToDelete.getListDesa() );
                pekerjaanRepository.delete(domainToDelete);               
            ...
            if (newDomain != null) {
                ...
                return "master/pekerjaan/pekerjaan_form";
            }

        } else if (operation.equals("edit_form")) {

            TbPekerjaan domain = pekerjaanRepository.findById(id);
            if (domain != null) {
                domain.setTempInt1(1); //0.New Form, 1.Edit Form, 3.Delete
                //Password tidak bisa di decode ya?


                // TbPekerjaanRoles domainDetil1 = new TbPekerjaanRoles();
                // TbPekerjaanRoles domainDetil2 = new TbPekerjaanRoles();
                // // for (TbPekerjaanRoles domainDetil: domain.getFuserRoles()) {
    ...
    @RequestMapping("/master/pekerjaan/perbaikan")
    @ResponseBody
