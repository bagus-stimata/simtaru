...
@Controller
@ComponentScan
public class TbStatusWilayahController {
    ...
    @RequestMapping("/master/statuswilayah")
    public String listIndex(final Model viewModel) {

        // final Authentication auth = SecurityContextHolder.getContext().getAuthentication();

  
        final TbStatusWilayah newDomain =new TbStatusWilayah();
        ...
        viewModel.addAttribute("allData", statusWilayahRepository.findAll());
        

        logger.info("# Form Task");
      
        return "master/statuswilayah/statuswilayah_list"; 
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = {"/master/statuswilayah_form/save_process"}, method = RequestMethod.POST)
    public String saveProcess(@ModelAttribute("domain") final TbStatusWilayah domain, @ModelAttribute("domainDetil1") final TbStatusWilayah domainDetil1, final RedirectAttributes redirectAttributes) {
        if (domain.getTempInt1()==0) {
            TbStatusWilayah newDomain = new TbStatusWilayah();
            ...
            if (statusWilayahRepository.save(newDomain) != null) {
            ...
            return "redirect:/master/statuswilayah/edit_form/" + newDomain.getId();

        }else if (domain.getTempInt1()==1) {
            TbStatusWilayah domainUpdate = statusWilayahRepository.findById(domain.getId() );
            ...
            statusWilayahRepository.save(domainUpdate);

            redirectAttributes.addFlashAttribute("saveUser", "success");
            return "redirect:/master/statuswilayah/edit_form/" + domain.getId();
        }
       
        
        return "redirect:/master/statuswilayah/";
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = "/master/statuswilayah/{operation}/{id}", method = RequestMethod.GET)
    public String toFormOperation(@PathVariable("operation") final String operation,
        ...
        logger.info("/master/statuswilayah/operation: {} ", operation);

       if (operation.equals("delete")) {
            try {
                TbStatusWilayah domainToDelete = statusWilayahRepository.findById(id);        
                // hati-hati kecamatanRepository.deleteAll(domainToDelete.getListDesa() );
                statusWilayahRepository.delete(domainToDelete);               
            ...
            if (newDomain != null) {
                ...
                return "master/statuswilayah/statuswilayah_form";
            }

        } else if (operation.equals("edit_form")) {

            TbStatusWilayah domain = statusWilayahRepository.findById(id);
            if (domain != null) {
                domain.setTempInt1(1); //0.New Form, 1.Edit Form, 3.Delete
                //Password tidak bisa di decode ya?


                // TbStatusWilayahRoles domainDetil1 = new TbStatusWilayahRoles();
                // TbStatusWilayahRoles domainDetil2 = new TbStatusWilayahRoles();
                // // for (TbStatusWilayahRoles domainDetil: domain.getFuserRoles()) {
    ...
    @RequestMapping("/master/statuswilayah/perbaikan")
    @ResponseBody
