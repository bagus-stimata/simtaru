...
@Controller
@ComponentScan
public class TipeLayananController {
    ...
    @RequestMapping("/master/tipelayanan")
    public String listIndex(final Model viewModel) {

        // final Authentication auth = SecurityContextHolder.getContext().getAuthentication();

  
        final TbTipeLayanan newDomain =new TbTipeLayanan();
        ...
        // viewModel.addAttribute("allData", new ArrayList<TbTipeLayanan>());

        logger.info("# Form Task");
      
        return "master/tipelayanan/tipelayanan_list"; 
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = {"/master/tipelayanan_form/save_process"}, method = RequestMethod.POST)
    public String saveProcess(@ModelAttribute("domain") final TbTipeLayanan domain, @ModelAttribute("domainDetil1") final TbTipeLayanan domainDetil1, final RedirectAttributes redirectAttributes) {
        if (domain.getTempInt1()==0) {
            TbTipeLayanan newDomain = new TbTipeLayanan();
            ...
            return "redirect:/master/tipelayanan/edit_form/" + newDomain.getId();

        }else if (domain.getTempInt1()==1) {
            TbTipeLayanan domainUpdate = tipeLayananRepository.findById(domain.getId() );
            ...
            redirectAttributes.addFlashAttribute("saveUser", "success");
            return "redirect:/master/tipelayanan/edit_form/" + domain.getId();
        }
       
        
        return "redirect:/master/tipelayanan/";
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = "/master/tipelayanan/{operation}/{id}", method = RequestMethod.GET)
    public String toFormOperation(@PathVariable("operation") final String operation,
        ...
        logger.info("/master/tipelayanan/operation: {} ", operation);

       if (operation.equals("delete")) {
            try {
                TbTipeLayanan domainToDelete = tipeLayananRepository.findById(id);        
            ...
            if (newDomain != null) {
                ...
                return "master/tipelayanan/tipelayanan_form";
            }

        } else if (operation.equals("edit_form")) {

            TbTipeLayanan domain = tipeLayananRepository.findById(id);
            if (domain != null) {
                domain.setTempInt1(1); //0.New Form, 1.Edit Form, 3.Delete
                //Password tidak bisa di decode ya?


                // TbTipeLayananRoles domainDetil1 = new TbTipeLayananRoles();
                // TbTipeLayananRoles domainDetil2 = new TbTipeLayananRoles();
                // // for (TbTipeLayananRoles domainDetil: domain.getFuserRoles()) {
    ...
    @RequestMapping("/master/tipelayanan/perbaikan")
    @ResponseBody
